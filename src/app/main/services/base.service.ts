  ...
  private loadCompare(): void {
    this.getAll().forEach(a => {
      ...
      a.forEach(b => {
        ...
        let lastMonth = new Date(b.issueDate.toDate().setMonth(b.issueDate.toDate().getMonth() - 1));
        let OneYearAgo = new Date(b.issueDate.toDate().setFullYear(b.issueDate.toDate().getFullYear() - 1));

        let key = ("0" + (today.getMonth() + 1)).toLocaleString().slice(-2) + "/" + today.getFullYear().toString();
        let keyM = ("0" + (lastMonth.getMonth() + 1)).toLocaleString().slice(-2) + "/" + lastMonth.getFullYear().toString();
        let keyY = ("0" + (OneYearAgo.getMonth() + 1)).toLocaleString().slice(-2) + "/" + OneYearAgo.getFullYear().toString();

        //console.log(key + ' M: ' + keyM + ' Y: ' + keyY);

        if (this.myMap.has(keyM)) {
          baseCompare.cLastMonth = this.getCompareDiff(b.consumption, this.myMap.get(keyM).c);
          baseCompare.vLastMonth = this.getCompareDiff(b.value, this.myMap.get(keyM).v);
        }
